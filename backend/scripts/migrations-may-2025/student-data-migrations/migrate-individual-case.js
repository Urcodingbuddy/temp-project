...
// 1. Find all Individucal cases
// 2. Loop through Individucal case
// 3. In each loop find student by using studentId from Individucal case record
// 4. in studentsJourney of student find records with classroomId of Individucal case record and oldClassroomId
...
// 7. from the record get the classroomId, SAY and academicYear and save it to Individucal case
...
async function runMigration() {
	...
	const allRecords = await IndividualRecord.find({}).lean()
	...
	if (bulkOperations.length > 0) {
		const result = await IndividualRecord.bulkWrite(bulkOperations)
		console.log(result)
	}

	console.log('delete bulk ------->', deletebulkOperations.length)
	if (deletebulkOperations.length > 0) {
		const result = await IndividualRecord.bulkWrite(deletebulkOperations)
		console.log(result)
	}
	...
	if (classNotFoundBullk.length > 0) {
		const result = await mongoose.connection
			.collection('removed_individual_cases')
