...
async function runMigration() {
	...
	const allRecords = await EducationPlanner.find({}).lean()
				...
				updateOne: {
					...
					update: {
						$set: {
							...
							createdAt: journey.dateTime,
							updatedAt: journey.dateTime,
						},
					},
				},
			})
	...
	if (bulkOperations.length > 0) {
		const result = await EducationPlanner.bulkWrite(bulkOperations)
		console.log(result)
	}

	console.log('delete bulk ------->', deletebulkOperations.length)
	if (deletebulkOperations.length > 0) {
		const result = await EducationPlanner.bulkWrite(deletebulkOperations)
		console.log(result)
	}
	...
	if (classNotFoundBullk.length > 0) {
		const result = await mongoose.connection
			.collection('removed_education_planners')
