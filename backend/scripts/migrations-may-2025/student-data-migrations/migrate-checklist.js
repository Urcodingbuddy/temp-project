...
async function runMigration() {
	...
	const checkListRecords = await StudentCheckList.find({}).lean()
	// console.log(checkListRecords.map((rec) => rec.studentId))
	const allStudents = await Students.find({
		_id: { $in: checkListRecords.map((rec) => rec.studentId) },
	}).lean()

	// console.log('----All Students----', allStudents.length)
	// console.log('----Observation Records----', checkListRecords.length)
	...
	for (const record of checkListRecords) {
		const student = allStudents.find(
			(st) => record.studentId && st._id.toString() === record.studentId.toString(),
		)
	...
	if (bulkOperations.length > 0) {
		const result = await StudentCheckList.bulkWrite(bulkOperations)
		console.log(result)
	}
	
	console.log('delete bulk ------->', deletebulkOperations.length)
	if (deletebulkOperations.length > 0) {
		const result = await StudentCheckList.bulkWrite(deletebulkOperations)
		console.log(result)
	}
	...
	if (classNotFoundBullk.length > 0) {
		...
		.collection('removed_checklist_records')
