...
// 1. Find all baselines
// 2. Loop through baseline
// 3. In each loop find student by using studentId from baseline record
// 4. in studentsJourney of student find records with classroomId of baseline record and oldClassroomId
...
// 7. from the record get the classroomId, SAY and academicYear and save it to baseline
...
async function runMigration() {
	...
	const allRecords = await BaselineRecord.find({}).lean()
	...
	for (const record of allRecords) {
		...
		if (student.studentsJourney && student.studentsJourney.length > 0) {
			...
			if (!journey || journey.length === 0) {
				oldClassNotFound += 1
				// oldClassNotFoundData.push({
				// 	studentId: student._id.toString(),
				// 	journey: JSON.stringify(student.studentsJourney),
	...
	console.log('---OLd class data not found ----', oldClassNotFoundData)

	console.log(JSON.stringify(bulkOperations[0]?.updateOne))

	if (bulkOperations.length > 0) {
		const result = await BaselineRecord.bulkWrite(bulkOperations)
		console.log(result)
	}

	console.log('delete bulk ------->', deletebulkOperations.length)
	if (deletebulkOperations.length > 0) {
		const result = await BaselineRecord.bulkWrite(deletebulkOperations)
		console.log(result)
	}
	...
	if (classNotFoundBullk.length > 0) {
		const result = await mongoose.connection
			.collection('removed_baseline_records')
