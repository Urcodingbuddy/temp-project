...
async function runMigration() {
	...
	const allObservationRecords = await ObservationRecord.find({}).lean()
	// console.log(allObservationRecords.map((rec) => rec.studentId))
	const allStudents = await Students.find({
		_id: { $in: allObservationRecords.map((rec) => rec.studentId) },
	}).lean()

	// console.log('----All Students----', allStudents.length)
	// console.log('----Observation Records----', allObservationRecords.length)
	...
	for (const record of allObservationRecords) {
	...
	if (bulkOperations.length > 0) {
		const result = await ObservationRecord.bulkWrite(bulkOperations)
		console.log(result)
	}

	console.log('delete bulk ------->', deletebulkOperations.length)
	if (deletebulkOperations.length > 0) {
		const result = await ObservationRecord.bulkWrite(deletebulkOperations)
		console.log(result)
	}
	...
	if (classNotFoundBullk.length > 0) {
		const result = await mongoose.connection
			.collection('removed_observation_records')
