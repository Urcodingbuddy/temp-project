...
async function runMigration() {
	...
	const allRecords = await WellBeingAssessment.find({}).lean()
	...
	if (bulkOperations.length > 0) {
		const result = await WellBeingAssessment.bulkWrite(bulkOperations)
		console.log(result)
	}

	console.log('delete bulk ------->', deletebulkOperations.length)
	if (deletebulkOperations.length > 0) {
		const result = await WellBeingAssessment.bulkWrite(deletebulkOperations)
		console.log(result)
	}
	...
	if (classNotFoundBullk.length > 0) {
		const result = await mongoose.connection
			.collection('removed_student_wellbeing_records')
