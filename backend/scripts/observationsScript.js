const mongoose = require('mongoose')
const { Students } = require('../models/database/myPeegu-student')
const { ObservationRecord } = require('../models/database/myPeegu-observation')

async function addClassRoomIdToObservationRecords() {
	try {
		const mongoDB = 'mongodb+srv://mypeeguserver:fzhZb0U9zNjwJswY@mypeegu-dev.zle6nri.mongodb.net/mypeegu'
		await mongoose.connect(mongoDB, { useNewUrlParser: true, useUnifiedTopology: true }).catch((error) => {
			console.log('Unable to initiate Mongoose default connection, because of the error: %s', error.message)
			process.exit(0)
		})
		mongoose.set('debug', true)
		const observationRecords = await ObservationRecord.find({ classRoomId: { $exists: false } }).select('school studentId createdAt')
		const studentIds = observationRecords.map((record) => record.studentId)
		const students = await Students.find({ _id: { $in: studentIds } }).select('school classRoomId createdAt')
		console.log('observationRecords', observationRecords.length)

		let observationUpdates = []
		observationRecords.map((record) => {
			const student = students.find((student) => student?._id.toString() === record.studentId.toString())
			if (student) {
				console.log(record.studentId)
				observationUpdates.push({
					updateMany: {
						filter: { studentId: record.studentId }, // Filter by document ID
						update: {
							$set: { classRoomId: student.classRoomId },
						},
					},
				})

				// console.log('classRoomId added Successfully on Observations')
			} else {
				console.log('ClassRooms for student not found ')
			}
		})
		console.log('observationUpdates', observationUpdates.length)
		const result = await ObservationRecord.bulkWrite(observationUpdates)
		console.log('Bulk ObservationRecord update operation completed:', result)
	} catch (error) {
		console.error('Error updating Students Journey', error)
		throw new Error('Error updating Students Journey', error)
	}
}

;(async () => {
	try {
		await addClassRoomIdToObservationRecords()
		process.exit(0) // Exit with success
	} catch (error) {
		console.error(error)
		process.exit(1) // Exit with failure
	}
})()
